...
"Id[number(10,0)]",Name,Address[varchar(40)],"Income[number(20,2)]",Single[boolean()],DateOfBirth[date()],CreatedAt[timestamp()],CreatedTime[time()]
...
SELECT 1::NUMBER(10,0) AS ID, 'John'::VARCHAR(16777216) AS NAME, '123 Main St'::VARCHAR(40) AS ADDRESS, 100.1::NUMBER(20,2) AS INCOME, true::BOOLEAN AS SINGLE, '1990-01-15'::DATE AS DATEOFBIRTH, '2000-12-31 23:59:59'::TIMESTAMP(9) AS CREATEDAT, '23:59:59'::TIME(9) AS CREATEDTIME
UNION ALL
SELECT 2::NUMBER(10,0) AS ID, 'Jane'::VARCHAR(16777216) AS NAME, '456 Main St'::VARCHAR(40) AS ADDRESS, 200.2::NUMBER(20,2) AS INCOME, false::BOOLEAN AS SINGLE, '1985-12-25'::DATE AS DATEOFBIRTH, '1990-01-01 00:00:00'::TIMESTAMP(9) AS CREATEDAT, '00:00:00'::TIME(9) AS CREATEDTIME
UNION ALL
SELECT 3::NUMBER(10,0) AS ID, 'Kane'::VARCHAR(16777216) AS NAME, '901 Main St'::VARCHAR(40) AS ADDRESS, 300.3::NUMBER(20,2) AS INCOME, true::BOOLEAN AS SINGLE, '1975-07-07'::DATE AS DATEOFBIRTH, '1980-06-15 12:30:45'::TIMESTAMP(9) AS CREATEDAT, '12:30:45'::TIME(9) AS CREATEDTIME
	`)
